...
Need to combine the first digit and the last digit of a line to form a two-digit number (in that order)

So am thinking of reading a "calibrate.txt" files and iterate each line into an array and we store that array
then we use a for loop to read each array and get only the interger of that array then combine the first interger and last interger using combineDigit function and store that in an array too or sth
now we add all of the combine Digit together and get the resolution (10, 583, 845 = calibration value)

Ok now i test the current code it work fine, now i have to instead of printing it i want to output the line to an array instead 
